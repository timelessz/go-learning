...
// 熔断器学习,熔断器的三种状态,状态获取
//https://www.cnblogs.com/hualou/p/12087946.html
import (
	"fmt"
...
func main() {
	...
	configA := hystrix.CommandConfig{ //创建一个hystrix的config
		...
		MaxConcurrentRequests:  50,                    //控制最大并发数为5，并且在一个统计窗口内处理的请求数量达到阈值会调用我们传入的降级回调函数
	...
	for i := 0; i < 100; i++ {
		go func() {
			...
			if c.IsOpen() {
				fmt.Println("断路器打开") //判断熔断器是否打开，一旦打开所有的请求都会走fallback
			} else {
				fmt.Println("断路器关闭") //判断熔断器是否打开，一旦打开所有的请求都会走fallback
			}
			if c.AllowRequest() {
				fmt.Println("允许访问") //判断熔断器是否打开，一旦打开所有的请求都会走fallback
			} else {
				fmt.Println("不允许访问") //判断熔断器是否打开，一旦打开所有的请求都会走fallback
			}
		}()
	}
	wg.Wait()
